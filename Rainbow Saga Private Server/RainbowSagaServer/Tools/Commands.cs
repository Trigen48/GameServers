...
using System.Text;
using RainbowServer.Proto;
using RainbowServer.Handler;
...
namespace RainbowServer
{
    ...
    {
        const int Header = 2;
        public static byte[] Parse(ConnectionInfo c, RequestItem req)
        {

            /*      try
                  {*/

            //     System.IO.File.AppendAllText("c:\\z\\inky.txt", "In-> "++"\r\n");
            Console.WriteLine("in-> " + req.MsgID.ToString());
            c.ar = req.data;
            c.ot.Clear();
            ...
            switch (req.MsgID)
            {
                ...
                case MessageConsts.RpcID:
                    LoginZoneMessageListener.RpcID(c);
                    break;

                default:
                    System.IO.File.WriteAllBytes("c:\\oa\\newout.bin", c.ar.ReadBytes(c.ar.bytesAvailable));
                    //throw new Exception();
                    break;
            }

            // Console.WriteLine(req.MsgID);
            if (c.cmd.Count != 0)
            {
                uint rep = 2147483649 - (req.MsgType - 2147483665);
                int sub = 0;
                foreach (RequestItem itm in c.cmd)
                {
                    //  Console.WriteLine("Reply-> " + itm.MsgID.ToString());
                    c.ot.WriteUInt((uint)(rep - sub));
                    c.ot.WriteUInt(itm.MsgID);
                    c.ot.WriteUInt(itm.Length);
                    c.ot.WriteUInt(c.SessionID);

                    c.ot.WriteBytes(itm.data);
                }
                c.cmd.Clear();
            }


            //   System.IO.File.AppendAllText("c:\\z\\inky.txt", "Out-> " + rep.ToString() + "\r\n");
            /*   }
               catch(Exception ex)
               {
